...
object StereoSbs {
    ...
    fun alignAndCreateSbsJpegBytes(
        ...
        try {
            ...
            val affineOut: Mat = if (affineSmall != null && !affineSmall.empty() && scaleForFeatures < 0.999) {
                // When both images are uniformly scaled by s:
                // linear part stays same, translation scales by s. Convert translation back by /s.
                val a = Mat()
                ...
                affineSmall
            } ?: fallbackCenteredScaleAffine(
                ultraWidth = requireNotNull(ultraBgr).cols(),
                ultraHeight = requireNotNull(ultraBgr).rows(),
                wideWidth = requireNotNull(wideBgr).cols(),
                wideHeight = requireNotNull(wideBgr).rows(),
    ...
    private fun estimateUltraToWideAffineRansac(
        ...
        nFeatures: Int = 900,
        ratioTest: Float = 0.75f,
        ransacReprojThresholdPx: Double = 3.0,
        maxIters: Long = 2000,
        ...
        try {
            ...
            if (affine.empty() || inlierCount in 0..5) return null
            return affine
        } finally {
